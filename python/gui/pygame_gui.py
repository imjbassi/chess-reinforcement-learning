...
if __name__ == "__main__":
    ...
    try:
        ...
        # Initialize chess engine (C++ or Python fallback)
        if CPP_ENGINE_AVAILABLE:
            print("Using C++ chess engine for enhanced performance")
            chess_board = ChessBoard(white_to_move=True)
            chess_board.reset()
            use_cpp_engine = True
        else:
            print("Using Python chess engine (fallback)")
            use_cpp_engine = False
            
        # PYTHON-ONLY CHESS IMPLEMENTATION (Fallback when C++ unavailable)
        initial_position = {
            0: 'r', 1: 'n', 2: 'b', 3: 'q', 4: 'k', 5: 'b', 6: 'n', 7: 'r',
            8: 'p', 9: 'p', 10: 'p', 11: 'p', 12: 'p', 13: 'p', 14: 'p', 15: 'p',
            48: 'P', 49: 'P', 50: 'P', 51: 'P', 52: 'P', 53: 'P', 54: 'P', 55: 'P',
        ...
        # Unified chess engine wrapper functions
        def get_legal_moves_unified(position_dict=None, white_to_move=True):
            """Get legal moves using C++ engine if available, otherwise Python fallback"""
            if use_cpp_engine:
                return chess_board.get_legal_moves()
            else:
                return get_simple_legal_moves(position_dict, white_to_move)
        
        def apply_move_unified(move_uci, position_dict=None):
            """Apply move using C++ engine if available, otherwise Python fallback"""
            if use_cpp_engine:
                chess_board.apply_move(move_uci)
                return position_dict  # C++ engine maintains internal state
            else:
                return apply_move(position_dict, move_uci)
        
        def is_game_over_unified(position_dict=None, white_to_move=True):
            """Check game over using C++ engine if available, otherwise Python fallback"""
            if use_cpp_engine:
                done, result = chess_board.is_game_over()
                if done:
                    result_msg = {0: "Draw", 1: "White wins!", -1: "Black wins!"}.get(result, "Game over")
                    return True, result_msg
                return False, ""
            else:
                return is_game_over(position_dict, white_to_move)
        
        def reset_game_unified():
            """Reset game state using C++ engine if available"""
            if use_cpp_engine:
                chess_board.reset()
            # For Python engine, we'll reset position in the game loop
        
        def get_position_dict_from_cpp():
            """Convert C++ board state to Python dictionary format for visualization"""
            if not use_cpp_engine:
                return initial_position.copy()
            
            # Get piece positions from C++ engine
            pieces_bitboards = chess_board._board.pieces()
            position_dict = {}
            
            # Map piece types to characters
            piece_chars = ['P', 'N', 'B', 'R', 'Q', 'K', 'p', 'n', 'b', 'r', 'q', 'k']
            
            for piece_idx, bitboard in enumerate(pieces_bitboards):
                for sq in range(64):
                    if bitboard & (1 << sq):
                        position_dict[sq] = piece_chars[piece_idx]
        ...
        for game_num in range(1, 3):
            ...
            reset_game_unified()
            if use_cpp_engine:
                position = get_position_dict_from_cpp()
            else:
            ...
            engine_type = "C++ Engine" if use_cpp_engine else "Python Engine"
            screen.blit(font.render(f"Game {game_num} - {engine_type}", True, (0, 200, 0)), (10, SIZE + 20))
            ...
            while not game_over and len(moves) < max_moves:
                ...
                # Generate legal moves using unified engine
                legal_moves = get_legal_moves_unified(position, white_to_move)
                
                # Check for game over conditions
                game_over, result = is_game_over_unified(position, white_to_move)
                if game_over:
                    break
                ...
                # Apply move using unified engine
                position = apply_move_unified(move, position)
                if use_cpp_engine:
                    # Update position from C++ engine for visualization
                    position = get_position_dict_from_cpp()
                moves.append(move)
